...
- ✅ **iOS platform pinned to 13.0** - Uncommented `platform :ios, '13.0'` in `ios/Podfile`. Required by `share_plus 13+`. *(Superseded 2026-09-20 — deployment target bumped to 15.0 and SPM enabled; see iOS: SPM & UIScene Migration below.)*
...
- ⚠️ **Isar Swift Package Manager gap** - `flutter build macos` warns that `isar_community_flutter_libs` has no SPM support and that this "will become an error in a future version of Flutter". Isar likewise stays on CocoaPods on iOS (see below).

### iOS: SPM & UIScene Migration (2026-09-20)
Generated by running a build/`pub get` under Flutter 3.47.5, which enables Swift Package Manager for iOS plugins and migrates the app to the UIScene lifecycle. All changes are confined to `ios/`.
- ✅ **iOS deployment target 13.0 → 15.0** - `ios/Podfile` (`platform :ios, '15.0'`) and `IPHONEOS_DEPLOYMENT_TARGET = 15.0` in all three Runner build configs (`ios/Runner.xcodeproj/project.pbxproj`). `MinimumOSVersion` removed from `ios/Flutter/AppFrameworkInfo.plist` (the target now lives only in the Xcode project).
- ✅ **SPM enabled for iOS plugins** - `ios/Runner.xcodeproj/project.pbxproj` gained a `FlutterGeneratedPluginSwiftPackage` local package (`Flutter/ephemeral/Packages/...`), added to the Runner target's `packageProductDependencies`, frameworks build phase, and the project's `packageReferences`.
